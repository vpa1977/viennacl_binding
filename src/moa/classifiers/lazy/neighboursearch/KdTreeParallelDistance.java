...
public class KdTreeParallelDistance extends KDTree {
		...
		if (m_min_range == null)
    {
      ...
      m_instance_types.commitBuffer();
		...
		{
			m_min_range.write(i* DirectMemory.DOUBLE_SIZE, m_EuclideanDistance.getRanges()[i][NormalizableDistance.R_MIN]);
			m_width.write(i*DirectMemory.DOUBLE_SIZE, m_EuclideanDistance.getRanges()[i][NormalizableDistance.R_WIDTH]);
		}
	...
	public void update(Instance instance) throws Exception {
	  ...
	  protected void findNearestNeighbours(Instance target, KDTreeNode node, int k,
          ...
          m_merge_sorter.sort(m_distance_results, m_sort_indices,  node.m_End-node.m_Start +1);
  ...
  private void readSortIndices(int[] ind) {
	...
	private void findDistances(int start, int end, Instance target, double max_length) {
		...
		if (m_distance_kernel == null)
		{
			...
			//m_distance_kernel.set_local_size(0, (int)Math.min(256, nextPow2(target.numAttributes())));
		}
