...
public class SimpleUpdater2 extends AbstractUtil implements Updater {
			...
			int num_batches)
	{
		...
		m_updater = ctx.getKernel("sgd_update", "simple_update2");
	...
	public synchronized  void applyUpdate(Buffer gradient_buffer, 
		...
		m_updater.set_arg(6, batch_number);
		m_updater.set_arg(7, m_learning_rate);
		m_updater.set_arg(8, m_weights);
		m_updater.set_arg(9, m_weights_delta);
		m_updater.invoke();
