 ...
 *
 */
public class FJLT extends AbstractUtil{
	...
	public FJLT(Context ctx, int n, int k)
	{
		...
		m_pow2_src = new Buffer(ctx, next_p2 * DirectMemory.DOUBLE_SIZE);
		m_pow2_src.fill((byte)0);
		m_srhst_constant = Math.sqrt(n/k);
		m_substitutions_buffer = new Buffer(ctx, k * DirectMemory.INT_SIZE);
		m_indicators_buffer = new Buffer(ctx, next_p2 * DirectMemory.DOUBLE_SIZE);
		m_transform_temp = new Buffer(ctx, next_p2* DirectMemory.DOUBLE_SIZE);
		m_pre_fft_temp = new Buffer(ctx, next_p2* DirectMemory.DOUBLE_SIZE);
		int[]  substitutions = new int[k];
		double[]  indicators = new double[next_p2];
		...
		if (ctx.memoryType() != ctx.MAIN_MEMORY)
		{
			...
			m_fix_nan_kernel = ctx.getKernel("fjlt", "fix_nan");
		}
		else
		{
	...
	private void initKernels(Context ctx)
	{
		...
		temp.append(loadKernel("fjlt.cl"));
